...
import { CategoryRounded, CloseRounded, MenuRounded } from "@mui/icons-material";
...
function Sidebar() {
  ...
  return (
    <>
        ...
        <ul className={"mt-20 flex flex-col"}>
          ...
          <li>
            <NavLink
              ...
              title={"Transactions"}
              icon={
                <TransactionsIcon active={checkIsActive("transactions")} />
              }
            />
          </li>
      ...
      <div onClick={() => setShow(true)} className=" md:hidden flex absolute top-5 right-3 justify-end p-2 rounded-full bg-white">
        {!show && <MenuRounded />}
      </div>
      {show && (
        <div className="flex fixed top-0 z-[100] w-full bg-black bg-opacity-50 h-screen block lg:hidden">
          ...
          <button  onClick={() => setShow(false)} className="h-12 flex justify-center items-center w-12 relative top-5 -right-2  p-2 rounded-full bg-white">
            <CloseRounded />
          </button>
        </div>
      )}
